#!/usr/bin/env python
import rospy
import tf
from std_msgs.msg import String, Header
import sensor_msgs

from geometry_msgs.msg import Twist
from math import sqrt, cos, sin, pi, atan2
import numpy
import sys
from geometry_msgs.msg import Twist
from geometry_msgs.msg import Vector3

class PID:
    def __init__(self, Kp, Td, Ti, dt):
        self.Kp = Kp
        self.Td = Td
        self.Ti = Ti
        self.curr_error = 0
        self.prev_error = 0
        self.sum_error = 0
        self.prev_error_deriv = 0
        self.curr_error_deriv = 0
        self.control = 0
        self.dt = dt

    def _compute_i_value(self):
        self.sum_error += self.curr_error * self.dt

    def update_control(self, current_error, reset_prev=False):
        # todo: implement this
        self.curr_error = current_error
        self.curr_error_deriv = (self.curr_error - self.prev_error) / self.dt

        # compute the value
        p_value = self.Kp * self.curr_error

        # compute the i value
        # sum error represent the integral, using trapzoid method
        self.sum_error += ((self.curr_error + self.curr_error) / 2) * self.dt
        i_value = self.sum_error * self.Ti

        # compute the d value, with error filetering
        a = 0.5
        filtered = a * self.prev_error_deriv + (1 - a) * self.curr_error_deriv
        d_value = self.Td * self.curr_error_deriv
        d_value = self.Td * filtered


        # save the error and d_error data
        self.prev_error = self.curr_error
        self.prev_error_deriv = self.curr_error_deriv

        # compute the turning angle
        angle = p_value
        angle += i_value
        angle += d_value

        print('p_value - %s  d_value - %s  i_value - %s' % (p_value, d_value, i_value))
        return angle


    def get_control(self):
        return self.control

class WallFollowerHusky:
    def __init__(self):
        rospy.init_node('wall_follower_husky', anonymous=True)

        self.forward_speed = rospy.get_param("~forward_speed")
        self.desired_distance_from_wall = rospy.get_param("~desired_distance_from_wall")
        self.hz = 5


        # todo: set up the command publisher to publish at topic '/husky_1/cmd_vel'
        # using geometry_msgs.Twist messages
        self.cmd_pub = rospy.Publisher('/husky_1/cmd_vel', Twist, queue_size=10)


        # todo: set up the laser scan subscriber
        # this will set up a callback function that gets executed
        # upon each spinOnce() call, as long as a laser scan
        # message has been published in the meantime by another node
        self.laser_sub = rospy.Subscriber("/husky_1/scan", sensor_msgs.msg.LaserScan, self.laser_scan_callback)

        # PID Controller init
        self.pid_controller = PID(0.8, 0.9, 0.2, 0.2)
        # self.desired_distance_from_wall = 3

    def laser_scan_callback(self, msg):
        # todo: implement this
        # Populate this command based on the distance to the closest
        # object in laser scan. I.e. compute the cross-track error
        # as mentioned in the PID slides.


        # TASK: compute the cross-track error
        left_side = len(msg.ranges) / 2
        error_value = min(msg.ranges[:left_side]) - self.desired_distance_from_wall
        error_msg = 'cross-track error: %s' % error_value
        print(error_msg)

        error_publisher = rospy.Publisher('/husky_1/cte', String, queue_size=1000)
        error_publisher.publish(error_msg)


        # You can populate the command based on either of the following two methods:
        # (1) using only the distance to the closest wall
        # (2) using the distance to the closest wall and the orientation of the wall
        #
        # If you select option 2, you might want to use cascading PID control.

        # cmd.angular.z = ???
        angle = self.pid_controller.update_control(error_value)
        print('updated control: %s' % angle)
        linear = Vector3((self.forward_speed), 0, 0)
        angular = Vector3(0, 0, angle)



        twist = Twist(linear,angular)
        self.cmd_pub.publish(twist)
        print('----------------------------------------------------')



    def run(self):
        rate = rospy.Rate(self.hz)
        rospy.spin()
        while not rospy.is_shutdown():
            rate.sleep()


if __name__ == '__main__':
    wfh = WallFollowerHusky()
    wfh.run()
